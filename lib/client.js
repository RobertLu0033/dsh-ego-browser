...
function mountFloatingWatch(ctx) {
	...
	*/
	...
	const openStream = () => {
		...
		doConnected = false;
		sse = new EventSource("/api/ego/stream", { withCredentials: true });
		sse.onopen = () => {
			doConnected = true;
		};
...
LivePreviewController.prototype.openStream = function() {
	...
	try {
		this.sse = new EventSource("/api/ego/stream", { withCredentials: true });
	} catch (e) {
		return;
	}
...
function mountSidebarTab(ctx, betterSidebar) {
	...
	var probeSse = null;
	try {
		probeSse = new EventSource("/api/ego/stream", { withCredentials: true });
	} catch (e) {}
